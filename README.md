...
### 이가람
#### 기획 과정
- 일단 먼저 아이디어를 생각해서 기획하는 것이 어려웠습니다. 그리고 아이디어를 생각하고 나서 필요한 데이터들을 생각하면서 ERD를 작성하는 과정이 두번째 어려움이었습니다. 테이블 마다 관계도 생각해야하고 정규화와 비정규화를 고려하면서 테이블을 작성해야 하는데, 처음 작성하다 보니 그런 점들을 고려하기 어려웠습니다. 

#### 프로젝트 진행
- 백엔드 업무를 수행할 때 url을 명시적으로 작성하고, 함수명, 변수명, 필드명 등등 모두 알아보기 쉽고 정확한 의미를 가진 이름으로 작성해야 하는데 그런 점이 쉽지 않았습니다. 테이블과 필드는 작성한 ERD를 보면서 만들었기 때문에 그렇게 큰 어려움은 없었지만 작업을 하다가 필드를 추가하고, 새로운 테이블을 만들어주고 하는 과정도 있었습니다. 
- DB의 데이터를 시리얼라이저를 통해 직렬화를 해서 프론트에 넘겨 줄 때 처음에는 프론트에서 처리하기 쉽게 건네주지 못하였습니다. 백엔드 쪽에서 데이터를 넘겨줄 때 그런 점을 고려하지 못하면 프론트에서 처리하기 어렵다는 것을 느끼고 중첩 시리얼라이저도 활용을 많이 하여서 세부 정보를 포함시키고, 불필요한 데이터를 전달하지 않으려고 했습니다. 그리고 에러가 발생했을 때 일관된 형식의 응답을 반환하려고 하였습니다. 

- drf_all_auth 시스템을 사용했는데 User의 새로운 필드를 추가하고, 그 데이터를 회원가입에 사용할려면 drf가 기본적으로 제공하는 serializer와 AccountAdapter를 수정할 때 처음에는 어려움을 겪었지만 문서와 pdf에 나와있는 예시를 보면서 완성을 할 수 있었습니다.

- 컴포넌트 내에서 데이터를 받아오는데 새로고침해야 페이지의 데이터가 렌더링이 되는 오류가 자꾸 생겨 디버깅 하는데 있어서 어려움을 겪었습니다. watch, async, await, beforerouteupdate등을 통해 그런 점들을 해결할 수 있었습니다. 

- await와 async를 사용하는데 있어서 비동기 처리와 동기 처리에 대해 이론적으로는 알고 있지만 적재적소에 사용하기가 어려웠습니다. 
,이스케이프 문자가 포함된 답변을 들고 와서 예외 처리를 해주어야 했습니다. 예외 처리를 해도 한 번씩 프롬프트에서 지정한 형태로 답변을 하지 않는 경우가 생겨서 해결하는데 어려움을 겪었습니다. 

- 첫 프로젝트라서 어떻게 진행해야 할지 갈피를 못잡았는데 유지인 팀원의 결단력과 아이디어로 기획 과정에서 빠르게 큰 틀을 잡을 수 있었던 것 같습니다. 그리고 초반 프론트 업무를 빠르게 한 덕분에 구현해보고 싶었던 기능들을 대부분 성공적으로 구현할 수 있어서 좋았습니다. 백엔드 과정을 끝내고 프론트를 같이 하는데 DB에 있는 데이터를 페이지에 띄우고, 경우에 따라 다른 화면이 보여아 하고, 사용자 체크도 해야하는데 정말 많은 체크 과정이 필요하다는 것을 느꼈고, 백엔드도 프론트의 업무를 잘 알아야하고, 프론트도 백엔드의 업무를 잘 알아야 프로젝트를 하는데 있어서 효율적인 작업이 수행될 거라는 것을 느꼈습니다.  
#### 기획 과정
